  ...
  return(
   <section>
      ...
      <div className="flex gap-4 p-1 overflow-x-auto pb-4">
        {myPets.map((pet) => (
          <div key={pet._id} className="flex-none w-48 border border-gray-200 text-center rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow">
        ...
        {myPets.length === 0 && (
          <p className="text-gray-500 w-full text-center py-4">Nenhum pet encontrado.</p>
        )}
      </div>
    </section>
  ) ;
